...
class RodNMod:
    ...
    def launchWebfishing(self, vanilla: bool = False):
        ...
        try:
            if vanilla:
                os.rename(mods_folder, disabled_folder)
            else:
                os.rename(disabled_folder, mods_folder)
    ...
    def searchModFolders(self, folderName: str):
        ...
        # Preprocess folder base names once to avoid redundant processing
        folder_base_names = [os.path.basename(folder) for folder in subs]
        
        # Run each transformation and match attempt
